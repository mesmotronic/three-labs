...
import { MediaPipeOrbitControls } from 'mediapipe-orbit-controls';
...
function initGraphics() {
  ...
  // Replace OrbitControls with MediaPipeOrbitControls
  mpControls = new MediaPipeOrbitControls({ camera, target: new THREE.Vector3(0, 2, 0), numHands: 2 });
  mpControls.init();

  mpControls.video.style.position = 'absolute';
  mpControls.video.style.top = '0';
  mpControls.video.style.right = '0';
  mpControls.video.style.width = '160px';
  mpControls.video.style.height = '120px';
  document.body.appendChild(mpControls.video);
...
function initInput() {
  // Replace pointerdown with MediaPipeOrbitControls 'pinchstart' event
  if (mpControls) {
    mpControls.addEventListener('pinchstart', function (event) {
      if (event.handIndex !== 1) return; // Use only right hand for shooting

      // Get cursor position in world space
      const cursorWorldPos = mpControls.cursor.getWorldPosition(new THREE.Vector3());
      // Get camera direction in world space
      const cameraWorldDir = new THREE.Vector3();
      camera.getWorldDirection(cameraWorldDir);
      // Shoot ball from cursor position in camera's forward direction
      const ballMass = 35;
      ...
      pos.copy(cursorWorldPos);
      quat.set(0, 0, 0, 1);
      const ballBody = createRigidBody(ball, ballShape, ballMass, pos, quat);
      // Set velocity in camera's forward direction
      const velocity = cameraWorldDir.clone().normalize().multiplyScalar(24);
